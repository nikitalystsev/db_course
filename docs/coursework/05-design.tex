...
В таблицах \ref{tbl:entity_user} -- ... приведены ограничения целостности для каждой таблицы разрабатываемой БД.
...
\includeimage
{trigger-avg-rating} % Имя файла без расширения (файл должен быть расположен в директории inc/img/)
{f} % Обтекание (без обтекания)
{h} % Положение рисунка (см. figure из пакета float)
{1\textwidth} % Ширина рисунка
